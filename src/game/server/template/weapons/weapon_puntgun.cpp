...
void CGrabController::DetachEntity(bool bClearVelocity)
{
	...
	if (pEntity)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				ClampPhysicsVelocity(pPhys, player_normspeed.GetFloat() * 1.5f, 2.0f * 360.0f);
			}
...
class CWeaponPuntGun : public CBaseSDKCombatWeapon
{
public:
	DECLARE_CLASS(CWeaponPuntGun, CBaseSDKCombatWeapon);
...
bool CWeaponPuntGun::m_sbStaticPoseParamsLoaded = false;
int CWeaponPuntGun::m_poseActive = 0;

IMPLEMENT_SERVERCLASS_ST(CWeaponPuntGun, DT_WeaponPhysCannon)
//SendPropBool(SENDINFO(m_bIsCurrentlyUpgrading)),
//SendPropFloat(SENDINFO(m_flTimeForceView)),
END_SEND_TABLE()

LINK_ENTITY_TO_CLASS(weapon_physcannon, CWeaponPuntGun);
PRECACHE_WEAPON_REGISTER(weapon_physcannon);

BEGIN_DATADESC(CWeaponPuntGun)
...
{
	//return (CGameRules()->MegaPhyscannonActive() == true);
	return false;
}
...
// Constructor
//-----------------------------------------------------------------------------
CWeaponPuntGun::CWeaponPuntGun(void)
{
...
// Purpose: Precache
//-----------------------------------------------------------------------------
void CWeaponPuntGun::Precache(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::Spawn(void)
{
	BaseClass::Spawn();
...
// On Remove
//-----------------------------------------------------------------------------
void CWeaponPuntGun::UpdateOnRemove(void)
{
	DestroyEffects();
...
// Sprite scale factor 
//-----------------------------------------------------------------------------
inline float CWeaponPuntGun::SpriteScaleFactor()
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponPuntGun::Deploy(void)
{
	CloseElements();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::SetViewModel(void)
{
	if (!IsMegaPhysCannon())
	{
...
//-----------------------------------------------------------------------------
void CWeaponPuntGun::ForceDrop(void)
{
	CloseElements();
	DetachObject();
	StopEffects();
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponPuntGun::DropIfEntityHeld(CBaseEntity *pTarget)
{
	if (pTarget == NULL)
		return false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::Drop(const Vector &vecVelocity)
{
	ForceDrop();
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CWeaponPuntGun::CanHolster(void)
{
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponPuntGun::Holster(CBaseCombatWeapon *pSwitchingTo)
{
	ForceDrop();
	DestroyEffects();
	//Don't holster this weapon if we're holding onto something
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DryFire(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::PrimaryFireEffect(void)
{
	CBasePlayer *pOwner = ToBasePlayer(GetOwner());
...
// Punt non-physics
//-----------------------------------------------------------------------------
void CWeaponPuntGun::PuntNonVPhysics(CBaseEntity *pEntity, const Vector &forward, trace_t &tr)
{
	float flDamage = 1.0f;
...
// What happens when the physgun picks up something 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::Physgun_OnPhysGunPickup(CBaseEntity *pEntity, CBasePlayer *pOwner, PhysGunPickup_t reason)
{
	// If the target is debris, convert it to non-debris
...
// Punt vphysics
//-----------------------------------------------------------------------------
void CWeaponPuntGun::PuntVPhysics(CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr)
{
	CTakeDamageInfo	info;
...
// Input  : - 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::ApplyVelocityBasedForce(CBaseEntity *pEntity, const Vector &forward, const Vector &vecHitPos, PhysGunForce_t reason)
{
...
// Punt non-physics
//-----------------------------------------------------------------------------
void CWeaponPuntGun::PuntRagdoll(CBaseEntity *pEntity, const Vector &vecForward, trace_t &tr)
{
	CBasePlayer *pOwner = ToBasePlayer(GetOwner());
...
// Trace length
//-----------------------------------------------------------------------------
float CWeaponPuntGun::TraceLength()
{
	if (!IsMegaPhysCannon())
	{
...
//-----------------------------------------------------------------------------
bool CWeaponPuntGun::EntityAllowsPunts(CBaseEntity *pEntity)
{
...
//-----------------------------------------------------------------------------
void CWeaponPuntGun::PrimaryAttack(void)
{
...
//-----------------------------------------------------------------------------
void CWeaponPuntGun::SecondaryAttack(void)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::WeaponIdle(void)
{
...
// Input  : *pObject - 
//-----------------------------------------------------------------------------
bool CWeaponPuntGun::AttachObject(CBaseEntity *pObject, const Vector &vPosition)
{
	if (m_bActive)
		return false;
	...
	if (pOwner)
	{
		...
		float	maxSpeed = player_walkspeed.GetFloat() + ((player_normspeed.GetFloat() - player_walkspeed.GetFloat()) * loadWeight);
...
void CWeaponPuntGun::FindObjectTrace(CBasePlayer *pPlayer, trace_t *pTraceResult)
{
	Vector forward;
...
CWeaponPuntGun::FindObjectResult_t CWeaponPuntGun::FindObject(void)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CBaseEntity *CWeaponPuntGun::MegaPhysCannonFindObjectInCone(const Vector &vecOrigin,
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
CBaseEntity *CWeaponPuntGun::FindObjectInCone(const Vector &vecOrigin, const Vector &vecDir, float flCone)
{
	// Find the nearest physics-based item in a cone in front of me.
...
bool CGrabController::UpdateObject(CBasePlayer *pPlayer, float flError)
{
	...
	/*if (CGameRules()->MegaPhyscannonActive())
	{
...
void CWeaponPuntGun::UpdateObject(void)
{
	CBasePlayer *pPlayer = ToBasePlayer(GetOwner());
	Assert(pPlayer);
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DetachObject(bool playSound, bool wasLaunched)
{
	if (m_bActive == false)
		return;
	...
	{
		//pOwner->EnableSprint(true);
		pOwner->SetMaxSpeed(player_normspeed.GetFloat());
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponPuntGun::ItemPreFrame()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::CheckForTarget(void)
{
...
// Begin upgrading!
//-----------------------------------------------------------------------------
void CWeaponPuntGun::BeginUpgrade()
{
	if (IsMegaPhysCannon())
		return;
	...
	SetContextThink(&CWeaponPuntGun::WaitForUpgradeThink, gpGlobals->curtime + 6.0f, s_pWaitForUpgradeContext);
...
// Wait until we're done upgrading
//-----------------------------------------------------------------------------
void CWeaponPuntGun::WaitForUpgradeThink()
{
	...
	{
		SetContextThink(&CWeaponPuntGun::WaitForUpgradeThink, gpGlobals->curtime + 0.1f, s_pWaitForUpgradeContext);
		return;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoEffectIdle(void)
{
	if (IsEffectActive(EF_NODRAW))
	{
		StopEffects();
...
//-----------------------------------------------------------------------------
void CWeaponPuntGun::ItemBusyFrame(void)
{
	DoEffectIdle();
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CWeaponPuntGun::ItemPostFrame()
{
	CBasePlayer *pOwner = ToBasePlayer(GetOwner());
	if (pOwner == NULL)
	{
...
void CWeaponPuntGun::LaunchObject(const Vector &vecDir, float flForce)
{
	// FIRE!!!
...
// Output : Returns true on success, false on failure.
//-----------------------------------------------------------------------------
bool CWeaponPuntGun::CanPickupObject(CBaseEntity *pTarget)
{
	if (pTarget == NULL)
		return false;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::OpenElements(void)
{
	if (m_bOpen)
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::CloseElements(void)
{
...
// Output : float
//-----------------------------------------------------------------------------
float CWeaponPuntGun::GetLoadPercentage(void)
{
...
// Output : CSoundPatch
//-----------------------------------------------------------------------------
CSoundPatch *CWeaponPuntGun::GetMotorSound(void)
{
	if (m_sndMotor == NULL)
	{
...
// Shuts down sounds
//-----------------------------------------------------------------------------
void CWeaponPuntGun::StopLoopingSounds()
{
	if (m_sndMotor != NULL)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DestroyEffects(void)
{
	//Turn off main glow
	if (m_hCenterSprite != NULL)
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::StopEffects(bool stopSound)
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::StartEffects(void)
{
	CBasePlayer *pOwner = ToBasePlayer(GetOwner());
	if (pOwner == NULL)
		return;
...
// Closing effects
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoEffectClosed(void)
{
	float flScaleFactor = SpriteScaleFactor();
...
// Closing effects
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoMegaEffectClosed(void)
{
	float flScaleFactor = SpriteScaleFactor();
...
// Ready effects
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoEffectReady()
{
	float flScaleFactor = SpriteScaleFactor();
...
// Holding effects
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoEffectHolding()
{
	float flScaleFactor = SpriteScaleFactor();
...
// Launch effects
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoEffectLaunch(Vector *pos)
{
	Assert(pos);
	if (pos == NULL)
		return;
...
// Input  : *pos - 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoMegaEffectLaunch(Vector *pos)
{
	Assert(pos);
	if (pos == NULL)
		return;
...
// Holding effects
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoMegaEffectHolding(void)
{
	float flScaleFactor = SpriteScaleFactor();
...
// Ready effects
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoMegaEffectReady(void)
{
	float flScaleFactor = SpriteScaleFactor();
...
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoEffectNone(void)
{
	if (m_hBlastSprite != NULL)
	{
		// Become small
...
//			*pos - 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoMegaEffect(int effectType, Vector *pos)
{
	switch (effectType)
	{
	case EFFECT_CLOSED:
...
// Input  : effectType - 
//-----------------------------------------------------------------------------
void CWeaponPuntGun::DoEffect(int effectType, Vector *pos)
{
...
// Output : const char
//-----------------------------------------------------------------------------
const char *CWeaponPuntGun::GetShootSound(int iIndex) const
{
...
//-----------------------------------------------------------------------------
void CWeaponPuntGun::RecordThrownObject(CBaseEntity *pObject)
{
...
void CWeaponPuntGun::PurgeThrownObjects()
{
	bool bListChanged;
...
bool CWeaponPuntGun::IsAccountableForObject(CBaseEntity *pObject)
{
...
{
	CWeaponPuntGun *pCannon = dynamic_cast<CWeaponPuntGun *>(pActiveWeapon);
	if (pCannon)
	{
...
{
	CWeaponPuntGun *pWeaponPhyscannon = assert_cast<	CWeaponPuntGun* >(pAnim);
...
{
	CWeaponPuntGun *pCannon = dynamic_cast<CWeaponPuntGun *>(pPhysCannon);
	if (pCannon)
	{
